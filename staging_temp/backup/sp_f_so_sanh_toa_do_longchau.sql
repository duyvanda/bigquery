CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_so_sanh_toa_do_longchau()
BEGIN 
 
TRUNCATE TABLE staging_temp.f_so_sanh_toa_do_longchau_temp;

INSERT INTO `staging_temp.f_so_sanh_toa_do_longchau_temp`

(   

-- Create or replace table staging_temp.f_so_sanh_toa_do_longchau_temp
...
(
  ...
  where channel ='TP' and hcotypeid ='NT'and statedescr ='Thành phố Hồ Chí Minh'
)
select 
a.*except(lng),
a.lng as lgn,
...
from `staging.d_manual_toa_do_nt_long_chau_hcm` a 
...
Create or replace table `warehouse.f_so_sanh_toa_do_longchau`

copy `staging_temp.f_so_sanh_toa_do_longchau_temp`;
