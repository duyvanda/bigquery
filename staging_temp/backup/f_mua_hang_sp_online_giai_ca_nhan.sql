CREATE PROCEDURE `spatial-vision-343005`.staging_temp.f_mua_hang_sp_online_giai_ca_nhan()
OPTIONS(
  strict_mode=false)
BEGIN 
 
 TRUNCATE TABLE `staging_temp.f_mua_hang_sp_online_giai_ca_nhan_temp`;

 INSERT INTO `staging_temp.f_mua_hang_sp_online_giai_ca_nhan_temp`

( 
-- Create or replace table staging_temp.f_mua_hang_sp_online_giai_ca_nhan_temp as
with 
...
select 
  ...
  sum(ke_hoach_ban_online) as sl_kh
from mapping a 
where channel ='TP'
group by 1,2,3,4,5
...
result_ca_nhan as (
select a.*,
rank() over (partition by ten_mien order by sl_pp_kh desc,sl_ban desc) as rank_canhan
 from summary a )

 select *,
 Case when ten_mien = 'Miền Nam' and rank_canhan <= 7 then 1500000
      when ten_mien = 'Miền Bắc' and rank_canhan <= 4 then 1500000
      else 0 end as giai_thuong_ca_nhan
  from result_ca_nhan 
);

Create or replace table `warehouse.f_mua_hang_sp_online_giai_ca_nhan`

copy `staging_temp.f_mua_hang_sp_online_giai_ca_nhan_temp`;
