CREATE PROCEDURE `spatial-vision-343005`.staging_temp.f_mua_hang_sp_online_giai_ca_nhan_tt()
OPTIONS(
  strict_mode=false)
BEGIN 
 
 TRUNCATE TABLE `staging_temp.f_mua_hang_sp_online_giai_ca_nhan_tt_temp`;

 INSERT INTO `staging_temp.f_mua_hang_sp_online_giai_ca_nhan_tt_temp`

( 
-- Create or replace table `staging_temp.f_mua_hang_sp_online_giai_ca_nhan_tt_temp` as
with 
...
cast ('2024-07-10' as date) as crtd_datetime,
crs as ma_crs,
...
sales as (
  ...
  Case when time(a.crtd_datetime) > time "17:30:00"  then date(a.crtd_datetime) + interval 1 day
  else date(a.crtd_datetime) end as crtd_datetime,
  Case
  when b.slsperid in (
                'MR1682KN',
                'MR2504',
...
select 
  ...
  a.ten_crm, 
  a.ten_mien,
  date(crtd_datetime) as crtd_datetime,
  min (crtd_datetime_min) as crtd_datetime_min,
  count(distinct ma_hco_tren_dms_pp) as sl_pp_kh,
  sum(sl_ban)  as sl_ban,
from mapping a 
where channel ='TP' and ma_hco_tren_dms_kpi is null
group by 1,2,3,4,5,6

),
result as (
select *,
rank() over (partition by crtd_datetime,ten_mien order by sl_pp_kh desc, sl_ban desc,crtd_datetime_min asc ) as rank_chot_don_than_toc
 from summary
 order by crtd_datetime 
)
 select *,
 Case when ten_mien = 'Miền Nam' and rank_chot_don_than_toc <=2 then 2000000
      when ten_mien = 'Miền Bắc' and rank_chot_don_than_toc = 1 then 2000000
      else 0 end as giai_thuong_ca_nhan_tt,
  current_datetime("+7") as inserted_at
  from result

);

Create or replace table `warehouse.f_mua_hang_sp_online_giai_ca_nhan_tt`

copy `staging_temp.f_mua_hang_sp_online_giai_ca_nhan_tt_temp`;
