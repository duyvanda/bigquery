CREATE PROCEDURE `spatial-vision-343005`.staging_temp.f_mua_hang_sp_online_giai_nhom()
OPTIONS(
  strict_mode=false)
BEGIN 
 
 TRUNCATE TABLE `staging_temp.f_mua_hang_sp_online_giai_nhom_temp`;

 INSERT INTO `staging_temp.f_mua_hang_sp_online_giai_nhom_temp`

( 
-- Create or replace table staging_temp.f_mua_hang_sp_online_giai_nhom_temp as
with 
...
select 
  ...
  a.ten_crm, 
  a.ten_mien,
  avg(he_so_nhom) as he_so_nhom,
  count(distinct ma_hco_tren_dms_pp) as sl_pp_kh,
  count(distinct ma_hco_tren_dms_kpi) as sl_kpi_kh,
  sum(sl_ban)  as sl_ban,
  sum(ke_hoach_ban_online) as sl_ban_kh
from mapping a 
where channel ='TP'
group by 1,2,3,4,5
...
crs_pp_nho_hon_10 as 
(
  select  ma_crm,ten_crm,min(sl_pp_kh) as sl_pp_kh_min from summary where sl_pp_kh < 10 and he_so_nhom = 1
  group by 1,2
)
,
...
select 
  ...
  a.ten_crm, 
  a.ten_mien,
  sum(he_so_nhom) as he_so_nhom,
  sum(sl_pp_kh) as sl_pp_kh ,
  sum(sl_kpi_kh) as sl_kpi_kh,
  sum(sl_ban)  as sl_ban,
  sum(sl_ban_kh) as sl_ban_kh
from summary a 
group by 1,2,3
)
,
result_crm as (
select a.*except(he_so_nhom),
Case when a.ma_crm ='MR0319' then 11 else he_so_nhom end as he_so_nhom,
Case when b.ma_crm is not null then 'False' else 'True' end as is_dat_dk_nhom, 
b.sl_pp_kh_min,
Case when a.ma_crm ='MR0319' then round(safe_divide(sl_pp_kh , 11),1) else
round(safe_divide(sl_pp_kh , he_so_nhom),1) end as sl_pp_kh_tb,

 from summary_crm a
  LEFT JOIN crs_pp_nho_hon_10 b on a.ma_crm = b.ma_crm
),
result as (
 select *,
 Case 
  when is_dat_dk_nhom ='True' then
      rank() over (partition by is_dat_dk_nhom ='True' order by sl_pp_kh_tb desc,sl_ban desc) 
  else null end as rank_nhom
  from result_crm
order by 3
)
select *,  
Case 
    when rank_nhom <=2 and ma_crm is not null then 5000000
else 0 end as giai_thuong_nhom
from result
);

Create or replace table `warehouse.f_mua_hang_sp_online_giai_nhom`

copy `staging_temp.f_mua_hang_sp_online_giai_nhom_temp`;
