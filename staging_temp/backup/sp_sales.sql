CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_sales()
BEGIN 
...
(   
-- Create or replace table `staging_temp.f_sales_crs_temp`
-- partition by date(ngaychungtu)
-- cluster by makhdms,sodondathang,mahd,manv
-- as
WITH data_pda AS (
    SELECT
        ordernbr,
...
data AS(
    SELECT
        ...
        --NOTE: 26/7 update hàng khuyến mãi k cộng số lượng
        CASE
        ...
        a.makenhkh,
        a.tenkenhkh,
        --NOTE: gặp các mã SP này thì đổi kênh
        CASE
        ...
        0 AS kpi_ds_pcl,
        --NOTE: SLPP Sản phẩm 1
        CASE
            --NOTE: Từ t10 -> t11 2024
        ...
        ---SLPP Sản phẩm 2
        CASE
            --NOTE: Từ t7 2024 k có tiêu chí sp2
            WHEN a.ngaychungtu >= '2024-07-01' 
            THEN NULL
            --note: từ t4 2024 đến t7 2024
            WHEN a.ngaychungtu >= '2024-04-01'
            ...
            AND sum(soluong) over (PARTITION by macongtycn, sodondathang, masanpham) >= 3 
            THEN IFNULL(makhcu, makhdms)
            --note: từ t1 2024 đến t4 2024   
            WHEN a.ngaychungtu >= '2024-01-01'
            AND ngaychungtu < '2024-04-01'
            AND masanpham IN ('T302105002')
            AND a.makenhkh = 'TP'
            AND doanhsochuavat > 0 THEN IFNULL(makhcu, makhdms)
            -- tháng 10 2023 chuyển sang kpi doanh số sản phẩm tt XPL
            WHEN ngaychungtu >= '2023-10-01' 
            THEN NULL 
            --NOTE: tháng 9 2023 tính tổng sl shema trên 1 đơn hàng >=3 mới tính 1 phân phối
            WHEN masanpham IN(
                'OH074',
                'OH075',
                'OH077',
            ...
            THEN IFNULL(makhcu, makhdms)
            --NOTE: tháng 7 2023 đổi medoral qua shema lá đôi
            WHEN masanpham IN(
                'OH074',
                'OH075',
                'OH077',
        ...
        CASE
            ...
            AND makenhkh = 'MT'
            AND masanpham = 'EH115'
            AND ngaychungtu >= '2024-01-01' THEN doanhsochuavat
            /* 
            Các rule còn lại là rule cũ
            */
            --NOTE: Thay đổi theo mốc time
            WHEN ngaychungtu < '2024-03-01'
    ...
    WHERE
        ...
        WHEN a.manv IN ('QUYNHPTA', 'MA001', 'MA002') AND makenhkh = 'OTH_LAB' THEN TRUE
        WHEN makhdms IN ('008140', '003589', '013410', '018851') THEN TRUE
        ELSE FALSE END
        )        
    ...
    ALL (
        SELECT
            ...
            CASE
                ...
                AND thang < '2025-07-01' THEN slkh_ebysta * 1000
                WHEN makenhkh = 'TP'
...
Create or replace table `warehouse.f_sales_crs` COPY `staging_temp.f_sales_crs_temp`;

-- CREATE OR REPLACE TABLE staging.f_sales_crs_partition_manv_int
-- PARTITION BY RANGE_BUCKET(manv_int, GENERATE_ARRAY(110000, 114000, 1))
-- AS
-- (
-- SELECT *, safe_cast(replace(manv, 'MR','11') as INT) as manv_int FROM `spatial-vision-343005.warehouse.f_sales_crs_temp`
-- );
