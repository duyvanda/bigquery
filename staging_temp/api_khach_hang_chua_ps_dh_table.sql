CREATE TABLE FUNCTION `spatial-vision-343005`.staging_temp.api_khach_hang_chua_ps_dh_table(p_ma_crs STRING)
AS
with
...
and starts_with(a.slsperid,p_ma_crs)
)
...
(
select makhdms,max(date(ngaychungtu)) as ngay_dat_don_gan_nhat from `warehouse.f_sales_crs` where date(ngaychungtu) between date(date_trunc(current_date("+7"),month)) and current_date("+7")
