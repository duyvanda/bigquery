CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_delivery_geo_map()
BEGIN 
  TRUNCATE TABLE staging_temp.f_delivery_geo_map_temp;

 INSERT INTO staging_temp.f_delivery_geo_map_temp(

-- Create table staging_temp.f_delivery_geo_map_temp
-- as
...
left join toado_base b on a.slsperid = b.manv
-- where geometry_base IS NULL and a.role_luong_mds = 'LOG'
--where a.tencvbh in ('Lê Quang Minh', 'Lê Hồng Phúc') and delivery = '2023-05-31' --and 


  );

Create or replace table `warehouse.f_delivery_geo_map`

copy `staging_temp.f_delivery_geo_map_temp`;
