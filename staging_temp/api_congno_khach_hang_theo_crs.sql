CREATE PROCEDURE `spatial-vision-343005`.staging_temp.api_congno_khach_hang_theo_crs(p_ma_crs STRING)
OPTIONS(
  strict_mode=false)
BEGIN
...
LEFT JOIN `staging.d_users` b on a.slsperid =b.manv
where slsperid not in ('GH001','QUYNHPTA','MA001','MA002') and tenquanlyvung='Nguyễn Thọ Chiến'
...
where contains_substr(concat(d.supid,slsperid), p_ma_crs) 
