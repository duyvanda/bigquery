CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_api_dontreo_cxs(p_manv STRING, p_version STRING)
BEGIN
-- Default values
...
BEGIN TRANSACTION;
TRUNCATE TABLE `spatial-vision-343005.warehouse.f_sales_crs_pending_v3_realtime`;
INSERT INTO `warehouse.f_sales_crs_pending_v3_realtime`
(
-- Create or replace table `warehouse.f_sales_crs_pending_v3_realtime` as
with 
...
(
 ...
 where errormessage not in ('tem.Byte[]') and errormessage is not null  and manv = p_manv and version = p_version
...
mapping as (
    ...
    from
        ...
        where a.manv =p_manv and a.version=p_version
...
select
    *
from
    result --where  crtd_user =p_manv or c.ma_crm =p_manv or ma_ncxm = p_manv
