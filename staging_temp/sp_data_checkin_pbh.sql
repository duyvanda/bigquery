CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_data_checkin_pbh()
BEGIN
...
(   
-- Create or replace table `staging_temp.f_data_checkin_pbh_temp`
-- partition by date(visitdate)
-- cluster by custid,slsperid,salesid
-- as
with 
nghiphep as (
SELECT distinct manvcsm,date(ngay) as ngay,
Case when extract(dayofweek from ngay) = 7 then 'P' else loainghiphep end as loainghiphep FROM `spatial-vision-343005.staging.d_manual_danhsach_nghiphep_pbh` 
where 
(
(Case when extract(dayofweek from ngay) = 7 then 'P' else loainghiphep end) like 'P%'
or (Case when extract(dayofweek from ngay) = 7 then 'P' else loainghiphep end) like 'T%'
)
and bophan in ('TP','MT','SDS')
),
...
tuyen_dms_moinhat as (
    with data_tuyen as (
        ...
        FROM
            ...
            left join `staging.d_master_khachhang_bytime` b on a.custid = b.custid and a.thang = b.thang
        where
            a.thang >='2024-01-01' and delroutedet is false and b.active='Active' 
