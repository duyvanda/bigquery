CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_thongtin_khachhang()
BEGIN
TRUNCATE TABLE staging_temp.f_thongtin_khachhang_temp;


INSERT INTO staging_temp.f_thongtin_khachhang_temp
(

-- CREATE OR REPLACE table staging_temp.f_thongtin_khachhang_temp
-- as 

---`view_report.pl|thông tin khách hàng`------
...
Case 
  ...
  when shoptype in ('CLC1','CLC2') then 'Không xét GDP'
  when channel in ('INS','NB','OTH_LAB') then 'Không xét GDP'
...
Create or replace table `warehouse.f_thongtin_khachhang`

copy `staging_temp.f_thongtin_khachhang_temp`;
