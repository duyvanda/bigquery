CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_nhacdon_tp()
BEGIN 
TRUNCATE TABLE staging_temp.f_nhacdon_tp_temp;
INSERT INTO staging_temp.f_nhacdon_tp_temp
(

-- Create or replace table `staging_temp.f_nhacdon_tp_temp`
-- as
...
(
  ...
  (
    ...
    (
      ...
      select 
        *,
        row_number()over(partition by makhdms order by ngaychungtu desc) as loc
      from ds_tp 
...
(
  select 
    ...
    b.masanpham, 
    b.tensanphamnb,
    case when a.manv = 'TMDT_001' then 'online' else 'offline' end as kenh_muahang,
  ...
  from f_sale a
  left join `spatial-vision-343005.staging.f_sales` b on a.macongtycn = b.macongtycn and a.sodondathang = b.sodondathang and a.mahd = b.mahd
)
,
result as
(
...
from result 
--  where ten_nvbh is null



);

Create or replace table `warehouse.f_nhacdon_tp`

copy `staging_temp.f_nhacdon_tp_temp`;
