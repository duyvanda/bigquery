CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_baocao_tonkho_hangngay_page_tonkhotonghop_v2_30112024()
BEGIN 
...
WITH data_vt AS (
    WITH data_ton_vt AS (
        SELECT 
            ...
            expdate,
            created_date,
            DATE(created_date) AS ngay_capture,
            toncuoi AS toncuoi
    ...
    SELECT 
        TIMESTAMP(ngay_capture) AS created_date,
        created_date AS inserted_at2,
        ...
        0 AS tonhangdiduong,
        0 AS tonvime,
        0 AS tonhangdiduongvime,
        0 AS AVG_7_ngay,
        0 AS SL_ban_MTD,
        0 AS AVG_30_ngay,
        0 AS avg_forecast,
        NULL AS colo,
        CAST(NULL AS STRING) AS handung,
        CAST(NULL AS STRING) AS congtykihdoanh,
        CAST(NULL AS STRING) AS leadtimesx_binhthuong,
        CAST(NULL AS STRING) AS leadtimesx_gap,
        CAST(NULL AS STRING) AS nhomcpa,
        CAST(NULL AS STRING) AS nhomcpa2,
        CAST(NULL AS STRING) AS brand2023,
        NULL AS songay_banhet1,
        NULL AS songay_banhet2,
        NULL AS songay_banhet3,
        NULL AS songay_banhet1_all,
        NULL AS songay_banhet2_all,
        NULL AS songay_banhet3_all,
        CAST(NULL AS STRING) AS is_check_sp,
        CAST(NULL AS STRING) AS is_check_sp_all,
        SUM(toncuoi) AS ton_vattu
    FROM data_ton_vt a
    LEFT JOIN `spatial-vision-343005.staging.d_dms_master_invtid` b 
        ON a.invtid = b.invtid
    GROUP BY created_date, inserted_at2, masanpham, chinhanh, tensanpham
),
...
base AS (
    ...
    WHERE 
        ...
        AND DATE(ngaychungtu) >= DATE_TRUNC(DATE_SUB(CURRENT_DATE("+7"), INTERVAL 3 MONTH), MONTH)
    GROUP BY masanpham, chinhanh
),
...
, group_base_tonkho AS (
    SELECT
        ...
        SUM(tonao) AS tonao
    FROM base_tonkho
    GROUP BY created_date, inserted_at2, masanpham, chinhanh_new
)
-- , base_date AS (
--     SELECT
--         *,
--         EXTRACT(DAYOFWEEK FROM day) AS week_day
--     FROM UNNEST(
--         GENERATE_DATE_ARRAY(
--             DATE_SUB(CURRENT_DATE("+7"), INTERVAL 24 MONTH),
--             DATE_ADD(CURRENT_DATE("+7"), INTERVAL 2 MONTH),
--             INTERVAL 1 DAY
--         )
--     ) AS day
-- )
...
--         SUM(
--             CASE
--                 WHEN week_day = 1 AND day <= CURRENT_DATE("+7") AND day > CURRENT_DATE("+7") - 30 THEN 0
--                 WHEN week_day = 7 AND day <= CURRENT_DATE("+7") AND day > CURRENT_DATE("+7") - 30 THEN 0.5
--                 WHEN week_day NOT IN (1, 7) AND day <= CURRENT_DATE("+7") AND day > CURRENT_DATE("+7") - 30 THEN 1
--                 ELSE NULL
--             END
--         ) AS songay_lamviec_trong30ngay,
--         SUM(
--             CASE
--                 WHEN week_day = 1 AND day <= CURRENT_DATE("+7") AND day > CURRENT_DATE("+7") - 7 THEN 0
--                 WHEN week_day = 7 AND day <= CURRENT_DATE("+7") AND day > CURRENT_DATE("+7") - 7 THEN 0.5
--                 WHEN week_day NOT IN (1, 7) AND day <= CURRENT_DATE("+7") AND day > CURRENT_DATE("+7") - 7 THEN 1
--                 ELSE NULL
--             END
--         ) AS songay_lamviec_trong7ngay,
--         SUM(
--             CASE
--                 WHEN week_day = 1 AND DATE_TRUNC(day, MONTH) = DATE_TRUNC(CURRENT_DATE("+7"), MONTH) THEN 0
--                 WHEN week_day = 7 AND DATE_TRUNC(day, MONTH) = DATE_TRUNC(CURRENT_DATE("+7"), MONTH) THEN 0.5
--                 WHEN week_day NOT IN (1, 7) AND DATE_TRUNC(day, MONTH) = DATE_TRUNC(CURRENT_DATE("+7"), MONTH) THEN 1
--                 ELSE NULL
--             END
--         ) AS songay_lamviec_trongthanghientai
--     FROM base_date
-- )
...
, fc_month_sales AS (
    ...
    FROM `spatial-vision-343005.staging.d_forecast_sc` t6
    WHERE DATE(t6.month) = DATE(DATE_TRUNC(CURRENT_DATE("+7"), MONTH))
      AND version = (
          SELECT MAX(version)
          FROM `staging.d_forecast_sc`
          WHERE DATE(month) = DATE(DATE_TRUNC(CURRENT_DATE("+7"), MONTH))
      )
    GROUP BY t6.month, t6.masp
),
sales_sp AS (
    SELECT
        masanpham,
...
fc_sales AS (
    SELECT
        tt1.masanpham,
        tt1.chinhanh,
        -- SỐ LƯỢNG TỪNG CHI NHÁNH / SỐ LƯỢNG TỔNG * TỔNG FC TRONG THÁNG
        (tt1.soluong_30ngay / tt2.soluong) * t2.fcvalues AS fc_chinhanh
    FROM base tt1
...
base_sanpham AS (
    SELECT
        ...
        a.descr,
        t2.* EXCEPT(masp)
    FROM `spatial-vision-343005.staging.d_dms_master_invtid` a
...
mapping_all AS (
    SELECT
        ...
        ROUND(tb2.fc_chinhanh / 26, 0) AS avg_forecast,
        t2.colosx AS colo,
        ...
        t3.brand2023
    FROM group_base_tonkho a
    FULL JOIN base b ON a.masanpham = b.masanpham AND LOWER(a.chinhanh) = LOWER(b.chinhanh)
    -- LEFT JOIN songay_lamviec cn1 ON 1 = 1
    FULL JOIN fc_sales tb2 ON COALESCE(a.masanpham, b.masanpham) = tb2.masanpham AND LOWER(COALESCE(a.chinhanh, b.chinhanh)) = LOWER(tb2.chinhanh)
