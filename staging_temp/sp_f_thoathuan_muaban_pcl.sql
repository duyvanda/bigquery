CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_thoathuan_muaban_pcl()
BEGIN 
  TRUNCATE TABLE staging_temp.f_thoathuan_muaban_pcl_temp;


 INSERT INTO staging_temp.f_thoathuan_muaban_pcl_temp(

-- Create or replace table staging_temp.f_thoathuan_muaban_pcl_temp
-- as
...
group_split as (
  ...
  FROM
    ...
    where discidpn in ( '202401-TL-QD974-PMC-CTD')
),
flattened as (
  select
    branchid,
...
, f_accumulatedresult_d as (select distinct ordernbr, custid from staging.f_accumulatedresult_d)
...
Create or replace table `warehouse.f_thoathuan_muaban_pcl`

copy `staging_temp.f_thoathuan_muaban_pcl_temp`;
