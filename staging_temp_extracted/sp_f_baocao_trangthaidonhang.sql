-- ==========================================================================
-- Routine Name : sp_f_baocao_trangthaidonhang
-- Routine Type : PROCEDURE
-- Dataset      : spatial-vision-343005.staging_temp
-- Created      : 2023-07-14 08:15:13.030000+00:00
-- Last Altered : 2023-07-14 08:15:13.030000+00:00
-- Extracted At : 2026-08-06 13:45:04
-- ==========================================================================

CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_baocao_trangthaidonhang()
BEGIN
  TRUNCATE TABLE staging_temp.f_baocao_trangthaidonhang_temp;

 INSERT INTO staging_temp.f_baocao_trangthaidonhang_temp(

-- Create table staging_temp.f_baocao_trangthaidonhang_temp
...
left join `spatial-vision-343005.staging.sync_dms_err` b on a.ordernbr = b.ordernbr
left join `spatial-vision-343005.staging.d_master_khachhang` c on a.custid = c.custid

where
(a.status_dv not in ('Đã giao hàng', 'Không tiếp tục giao hàng') or a.status_dv is null)
and a.status_pda_so not in ('Đóng đơn hàng tạm','Đóng đơn hàng')
and a.channel not in ('OTH_LAB','NB')
and a.trangthaidon not in ('Đóng đơn hàng','Hủy hóa đơn')
 );

Create or replace table `warehouse.f_baocao_trangthaidonhang`

copy `staging_temp.f_baocao_trangthaidonhang_temp`;
