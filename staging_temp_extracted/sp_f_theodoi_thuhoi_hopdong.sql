-- ==========================================================================
-- Routine Name : sp_f_theodoi_thuhoi_hopdong
-- Routine Type : PROCEDURE
-- Dataset      : spatial-vision-343005.staging_temp
-- Created      : 2025-09-25 08:34:23.798000+00:00
-- Last Altered : 2025-09-25 08:34:23.798000+00:00
-- Extracted At : 2026-08-06 13:45:04
-- ==========================================================================

CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_theodoi_thuhoi_hopdong()
BEGIN

Create or replace table `warehouse.f_theodoi_thuhoi_hopdong` as
...
cast (null as timestamp) as ngay_gia_han_thu_hoi
...
Case
  ...
  when date(ngay_hieu_luc_theo_hd) >= date('2023-12-01') and current_date("+7") >= COALESCE(date(ngay_gia_han_thu_hoi),date(ngay_den_han_thu_hoi_hd)) then 'ĐẾN HẠN'
  when date(ngay_hieu_luc_theo_hd) >= date('2023-12-01') and current_date("+7") < COALESCE(date(ngay_gia_han_thu_hoi),date(ngay_den_han_thu_hoi_hd)) then 'CHƯA ĐẾN HẠN'
...
from result a
-- LEFT JOIN staging.view_manual_hop_dong_truoc_2024_a_kha m on a.so_hop_dong = m.manual_hop_dong_truoc_2024_a_kha
);

-- Create or replace table `warehouse.f_theodoi_thuhoi_hopdong`
-- copy `staging_temp.f_theodoi_thuhoi_hopdong_temp`;
END;
