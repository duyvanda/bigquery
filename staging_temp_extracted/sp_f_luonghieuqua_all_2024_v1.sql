-- ==========================================================================
-- Routine Name : sp_f_luonghieuqua_all_2024_v1
-- Routine Type : PROCEDURE
-- Dataset      : spatial-vision-343005.staging_temp
-- Created      : 2026-02-03 06:24:49.031000+00:00
-- Last Altered : 2026-02-03 06:24:49.031000+00:00
-- Extracted At : 2026-08-06 13:45:04
-- ==========================================================================

CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_luonghieuqua_all_2024_v1()
BEGIN
...
-- select DISTINCT thang from data_asm_kpi where thang = '2026-01-01'
/*XÓA VÀ MERGE VỚI SUP DATA (data_sup_kpi)*/
-- , th_kh_fmcg_toan_quoc as
-- (
--   select
--   thang,
--   SUM(th_fmcg) as th_fmcg,
--   SUM(IFNULL(kpi_fmcg,0)) as kpi_fmcg,
--   SUM(IFNULL(th_kpi_fmcg,0)) as th_kpi_fmcg
--   from data_sup_kpi
--   where supid in ('MR3066','MR4010','MRKAM1','MR4037')
--     AND makenhkh = 'MT' and thang >= '2025-04-01'
--   GROUP BY ALL
-- )
...
(
  ...
  a.ho_ten_crs,
  CASE
  WHEN b.makenhkh = 'TP' THEN b.th_kpi * 0.8 + b.th_kpi_sptt * 0.2
  WHEN b.makenhkh = 'HCP' THEN b.th_kpi * 0.8 + (IFNULL(SAFE_DIVIDE(IFNULL(b.ds_pcl,0)+IFNULL(b.ds_clc,0), IFNULL(b.kh_total_pcl,0)+IFNULL(b.kh_total_clc,0)), 0) * 100 * 0.2)
  ELSE null END as th_kpi_support,
...
----Lương hiệu quả 3 kênh TP -	Phụ cấp kết quả công việc (LHQ3): Căn cứ vào N
    ...
    -- when th_kpi_slpp < 80 and makenhkh='TP' and a.thang <'2024-11-01'  then 0
    -- when th_kpi_slpp >= 80 and th_kpi_slpp < 90 and makenhkh='TP' and a.thang <'2024-11-01'  then 3000000
    -- when th_kpi_slpp >= 90 and th_kpi_slpp < 100 and makenhkh='TP'  and a.thang <'2024-11-01' then 4000000
    -- when th_kpi_slpp >= 100 and th_kpi_slpp < 110 and makenhkh='TP'  and a.thang <'2024-11-01' then 6000000
    -- when th_kpi_slpp >= 110 and th_kpi_slpp < 120 and makenhkh='TP' and a.thang <'2024-11-01'  then 7000000
    -- when th_kpi_slpp >= 120  and makenhkh='TP'  then 8000000
  else null
  end as lhq3,
  0 as lhq4
...
--   Case
--       -----Ngô Tiến Vũ
--     when b.th_dskh_pcl_toanquoc < 100  and a.manv ='MR0123'  then b.th_dskh_pcl_toanquoc * 8000000 / 100
--     when b.th_dskh_pcl_toanquoc < 110 and b.th_dskh_pcl_toanquoc >= 100  and a.manv ='MR0123'  then b.th_dskh_pcl_toanquoc * 12000000 / 100
--     when b.th_dskh_pcl_toanquoc >= 110  and a.manv ='MR0123'  then b.th_dskh_pcl_toanquoc * 16000000 / 100
--     -----Vũ Mừng
--     when b.th_dskh_ins_toanquoc < 100  and a.manv ='MR1137'  then b.th_dskh_ins_toanquoc * 16000000 / 100
--     when b.th_dskh_ins_toanquoc < 110 and b.th_dskh_ins_toanquoc >= 100  and a.manv ='MR1137'  then b.th_dskh_ins_toanquoc * 22000000 / 100
--     when b.th_dskh_ins_toanquoc >= 110  and a.manv ='MR1137'  then b.th_dskh_ins_toanquoc * 28000000 / 100
--   else null end as lhq2,
--   0 as lhq3,
--   0 as lhq4
--   FROM combined a
--   LEFT JOIN th_kh_ins_pcl_toan_quoc b on a.thang = b.thang
--   WHERE makenhkh = 'HCP' and dtype = 'crm' and manv in ('MR1137','MR0123')
-- )
-- , HCP_NCRM_aToan as
-- (
--   select a.*,
--   0 as lhq1,
--   Case
--     when a.th_kpi < 100  and a.manv='MR1681'  then a.th_kpi * 19000000 / 100
--     when a.th_kpi < 110 and a.th_kpi >= 100  and a.manv='MR1681'  then a.th_kpi * 24000000 / 100
--     when a.th_kpi >= 110  and a.manv='MR1681'  then a.th_kpi * 29000000 / 100
--   else null end as lhq2,
--   0 as lhq3,
--   0 as lhq4
--   FROM combined a
--   WHERE makenhkh = 'HCP' and dtype = 'asm'
-- )
/*XÓA*/
, HCP_NCRM as
(
...
(
  ...
  WHEN a.thang >= '2025-10-01' THEN
      (
      case
      --when f_kpi_mt(ds ca nhan, ds chung crm, kh ca nhan, kh crm) < 80
  ...
  -- and upper(trim(chucdanhengtitlesum)) like '%KAS%' ----Se bi mat MR3066_KN
)
, MT_NV_SUP as
(
  select a.*,
  ...
  END AS lhq2,
  CASE
  WHEN a.thang <= '2025-09-30' AND UPPER(TRIM(chucdanhengtitlesum)) = 'KAS (SI)' THEN
  (   CASE
      WHEN th_kpi_si >= 120 AND a.thang >= '2025-10-01' THEN 3000000
      WHEN th_kpi_si >= 110 AND a.thang >= '2025-10-01' THEN 2500000
      WHEN th_kpi_si >= 100 AND a.thang >= '2025-10-01' THEN 2000000
  ...
  WHEN UPPER(TRIM(chucdanhengtitlesum)) = 'KAS (FMCG)' THEN  -- Chưa có sup của SI
  (   CASE
...
(
  select a.*,
  ---LHQ1 cho kênh MT - KAM
  case
    when doanhsochuavat >= 6000000000 and a.manv in('MR3066','MR4010','MR4037')  then doanhsochuavat * 0.22 /100
    when doanhsochuavat >= 5500000000 and a.manv in('MR3066','MR4010','MR4037') then 11500000
    when doanhsochuavat >= 5000000000 and a.manv in('MR3066','MR4010','MR4037') then 10000000
    when doanhsochuavat >= 4500000000 and a.manv in('MR3066','MR4010','MR4037') then 8500000
    when doanhsochuavat >= 4000000000 and a.manv in('MR3066','MR4010','MR4037') then 7000000
    when doanhsochuavat < 4000000000 and makenhkh ='MT' and a.manv in('MR3066') then doanhsochuavat * 0.17 /100

  ---LHQ1 cho kênh MT - KAM căn cứ vào T và k có phụ trách kênh FMCG Nguyễn Thị Nga
    when doanhsochuavat >= 8000000000 and  a.manv ='MR0868'  then doanhsochuavat * 0.25 /100
    when doanhsochuavat >= 7500000000 and  a.manv ='MR0868'  then 19000000
    when doanhsochuavat >= 7000000000 and  a.manv ='MR0868'  then 17000000
    when doanhsochuavat >= 6500000000 and  a.manv ='MR0868'  then 15000000
    when doanhsochuavat >= 6000000000 and  a.manv ='MR0868'  then 13000000
    when doanhsochuavat <  6000000000 and  a.manv ='MR0868'  then doanhsochuavat * 0.2 /100
  else null end as lhq1,
  Case ----Lương hiệu quả 2 kênh TP - Phụ cấp kết quả công việc (LHQ2): Căn cứ vào A
    when th_kpi < 80 and a.manv in('MR3066','MR4010','MR4037') then th_kpi * 5000000 / 100
    when th_kpi >= 80 and th_kpi < 90   and a.manv in('MR3066','MR4010','MR4037') then th_kpi * 10000000 / 100
    when th_kpi >= 90 and th_kpi < 100  and a.manv in('MR3066','MR4010','MR4037') then th_kpi * 11000000 / 100
    when th_kpi >= 100 and th_kpi < 110 and a.manv in('MR3066','MR4010','MR4037') then th_kpi * 12000000 / 100
    when th_kpi >= 110 and th_kpi < 120 and a.manv in('MR3066','MR4010','MR4037') then th_kpi * 12500000 / 100
    when th_kpi >= 120 and makenhkh ='MT' and a.manv in('MR3066','MR4010','MR4037') then th_kpi * 13000000 / 100

  ---LHQ 2 kênh MT KAM phụ thuộc vào th_kpi và k có phụ trách kênh FMCG
    when th_kpi < 80 and a.manv ='MR0868' then th_kpi * 5000000 / 100
    when th_kpi >= 80 and th_kpi < 90   and a.manv ='MR0868' then th_kpi * 10000000 / 100
    when th_kpi >= 90 and th_kpi < 100  and a.manv ='MR0868' then th_kpi * 11000000 / 100
    when th_kpi >= 100 and th_kpi < 110 and a.manv ='MR0868' then th_kpi * 12000000 / 100
    when th_kpi >= 110 and th_kpi < 120 and a.manv ='MR0868' then th_kpi * 12500000 / 100
    when th_kpi >= 120 and a.manv ='MR0868' then th_kpi * 13000000 / 100
  else null end as lhq2,
  Case
  ---LHQ 3  kênh MT KAM
    when th_kpi_fmcg < 80 and makenhkh='MT' and a.manv in('MR3066','MR4010','MR4037') then 0
    when th_kpi_fmcg >= 80 and th_kpi_fmcg < 90  and makenhkh='MT'and a.manv in('MR3066','MR4010','MR4037') then 3000000
    when th_kpi_fmcg >= 90 and th_kpi_fmcg < 100 and makenhkh='MT' and a.manv in('MR3066','MR4010','MR4037') then 4000000
    when th_kpi_fmcg >= 100 and th_kpi_fmcg < 110 and makenhkh='MT' and a.manv in('MR3066','MR4010','MR4037') then 6000000
    when th_kpi_fmcg >= 110 and th_kpi_fmcg < 120 and makenhkh='MT' and a.manv in('MR3066','MR4010','MR4037') then 7000000
    when th_kpi_fmcg >= 120 and makenhkh ='MT' and a.manv in('MR3066','MR4010','MR4037') then 8000000

  ---LHQ 3  kênh MT KAM phụ thuộc vào N --Chị nga SPTT Ebysta
    when IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) < 80   and makenhkh='MT' and a.manv ='MR0868' then 0
    when IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) >= 80  and IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) < 90 and makenhkh='MT' and a.manv ='MR0868' then 3000000
    when IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) >= 90  and IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) < 100 and makenhkh='MT' and a.manv ='MR0868' then 4000000
    when IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) >= 100 and IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) < 110 and makenhkh='MT' and a.manv ='MR0868' then 6000000
    when IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) >= 110 and IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) < 120 and makenhkh='MT' and a.manv ='MR0868' then 7000000
    when IF(a.thang >= '2025-10-01',th_kpi_sptt_mt,th_kpi_fmcg ) >= 120 and makenhkh='MT' and a.manv ='MR0868' then 8000000
  else null
  end as lhq3,
  0 as lhq4

  from combined a
  WHERE makenhkh = 'MT' and dtype = 'crm' and manv in ('MR3066', 'MR0868','MR4010','MR4037')
)
...
-- UNION ALL
-- select * from HCP_NCRM_aMung_aVu
-- UNION ALL
-- select * from HCP_NCRM_aToan
UNION ALL
select * from MT_NV
UNION ALL
...
case when a.dtype = 'nv' and a.manv = u.supid then concat(a.manv, '_KN') else a.manv end as manv,
Case
...
WHERE
hr.bophanteam not in ('GT')
AND a.manv != 'MR0485' -- CRM kênh GT Nguyễn Hoàng Viển
)
