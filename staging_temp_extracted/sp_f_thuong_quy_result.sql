-- ==========================================================================
-- Routine Name : sp_f_thuong_quy_result
-- Routine Type : PROCEDURE
-- Dataset      : spatial-vision-343005.staging_temp
-- Created      : 2026-08-05 08:53:07.854000+00:00
-- Last Altered : 2026-08-05 08:53:07.854000+00:00
...
, tieu_chi_no_xau_crm AS (
    SELECT
        k.ma_nv,
        c.asm,
        EXTRACT(QUARTER FROM k.thang) AS quy,
        EXTRACT(YEAR FROM k.thang) AS nam,
        /* Chỉ lấy Tổng nợ xấu */
        SUM(CASE WHEN a.phanloai_no IN ('Nợ đỏ', 'Nợ đen') THEN SAFE_CAST(a.du_cuoi_ky_no AS FLOAT64) ELSE 0 END) AS tong_no_xau,
        /* Chỉ lấy Kế hoạch nợ xấu */
        MAX(SAFE_CAST(k.ke_hoach AS FLOAT64)) AS ke_hoach_no_xau
    FROM `spatial-vision-343005.staging.d_manual_gs_ke_hoach_thuong_quy_no_xau_2026` k
    ...
    LEFT JOIN `spatial-vision-343005.warehouse.view_cong_no_kt_thuong_quy_hcp` a
        ON a.ma_crm = k.ma_nv
        AND EXTRACT(QUARTER FROM a.thang ) = EXTRACT(QUARTER FROM k.thang)
        AND EXTRACT(YEAR FROM a.thang ) = EXTRACT(YEAR FROM k.thang)
    LEFT JOIN `staging.d_users_bytime` c ON c.manv = k.ma_nv AND date(k.thang) = date(c.thang)
    GROUP BY 1, 2, 3, 4
)
...
, tieu_chi_no_xau_ncrm AS (
    SELECT
        ...
        /* KH lấy trực tiếp từ bảng Kế hoạch */
        MAX(SAFE_CAST(k.ke_hoach AS FLOAT64)) AS ke_hoach_no_xau
    FROM tieu_chi_no_xau_crm c
    LEFT JOIN `spatial-vision-343005.staging.d_manual_gs_ke_hoach_thuong_quy_no_xau_2026` k
        ON k.ma_nv = c.asm
        AND EXTRACT(QUARTER FROM k.thang) = c.quy
        AND EXTRACT(YEAR FROM k.thang) = c.nam
    GROUP BY 1, 2, 3
)
, data_nv_kpi AS (
SELECT
