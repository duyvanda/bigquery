-- ==========================================================================
-- Routine Name : sp_f_dsdh_trathuong_ct_tichluy_q22025
-- Routine Type : PROCEDURE
-- Dataset      : spatial-vision-343005.staging_temp
-- Created      : 2025-08-05 07:42:31.856000+00:00
-- Last Altered : 2025-08-05 07:42:31.856000+00:00
-- Extracted At : 2026-08-06 13:45:04
-- ==========================================================================

CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_dsdh_trathuong_ct_tichluy_q22025()
BEGIN

-- TRUNCATE TABLE staging_temp.f_dsdh_trathuong_ct_tichluy_temp;
CREATE TEMP TABLE `f_dsdh_trathuong_ct_tichluy_temp_q22025`
...
WHERE
    ...
    (
    CASE
    WHEN remark LIKE '%MKT@25MTP2007%' AND remark LIKE '%Q2/25%' then true
    WHEN remark LIKE '%MKT@25MTP2006%' AND remark LIKE '%Q2/25%' then true
    WHEN remark LIKE '%MKT@25MTP2008%' AND remark LIKE '%Q2/25%' then true
    else false
    END
    )
)
...
where
true
AND a.inserted_at >= '2025-07-22 00:00:00 UTC'
...
Create or replace table `warehouse.f_dsdh_trathuong_ct_tichluy_q22025`

copy `f_dsdh_trathuong_ct_tichluy_temp_q22025`;
END;
