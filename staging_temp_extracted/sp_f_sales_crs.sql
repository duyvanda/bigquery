-- ==========================================================================
-- Routine Name : sp_f_sales_crs
-- Routine Type : PROCEDURE
-- Dataset      : spatial-vision-343005.staging_temp
-- Created      : 2026-07-31 04:39:12.342000+00:00
-- Last Altered : 2026-07-31 04:39:12.342000+00:00
-- Extracted At : 2026-08-06 13:45:04
-- ==========================================================================

CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_sales_crs(start_date STRING)
BEGIN

-- CREATE TEMP TABLE `f_sales_crs` PARTITION BY DATE(ngaychungtu) AS
-- (
With raw_data as
(
SELECT
...
ma_crm_dongnhat as crm,
ma_scrm_dongnhat as scrm,
ma_ncxm_dongnhat as ncxm,
tencvbh,
tenquanlytt_dongnhat as tenquanlytt,
tenquanlykhuvuc_dongnhat as tenquanlykhuvuc,
tenquanlyvung_dongnhat as tenquanlyvung,
ds_sp_thang,
CASE
    WHEN trang_thai_giao_hang = 'Đã giao hàng' Then 1 else 0 end as trang_thai_giao_hang,
a.pubcustid,
a.pubcustname
...
WHERE DATE(ngaychungtu) >= PARSE_DATE('%Y-%m-%d', start_date)
...
CASE
    ...
    WHEN (manv IN ('MR0868','MR1360') OR tenquanlytt IN ('Nguyễn Thị Nga'))
        AND makenhkh = 'MT' AND masanpham = 'EH115' AND ngaychungtu >= '2024-01-01'
        THEN doanhsochuavat
...
CASE
    ...
    WHEN a.htbh = 'MDS' AND thang <= '2023-01-01' THEN 'MDS'
    ELSE a.makenhkh
END AS makenh_moi,
null as makenhphu,
null as phong_kh,
...
END AS manv,
null as phanloai,
null as crs_tuyenbanhang_trongmcp,
u.supid as crm,
u.asm as scrm,
u.rsmid as ncxm,
...
CASE
    ...
    WHEN makenhkh = 'TP' AND thang >= '2025-05-01' AND thang <= '2025-06-30' THEN slkh_ebysta
...
END AS slkh_ebysta,
CASE
    WHEN makenhkh = 'TP' AND thang >= '2024-01-01' THEN ROUND(slkh_ladoi, 1)
...
CASE
    ...
    WHEN makenhkh = 'TP' AND thang >= '2026-06-01' AND a.thang <= '2026-06-30' THEN slkh_ebysta
    WHEN makenhkh = 'TP' AND thang >= '2026-05-01' AND a.thang <= '2026-05-31' THEN slkh_ebysta * 1000
    WHEN makenhkh = 'TP' AND thang >= '2026-03-01' AND thang <= '2026-04-30' THEN slkh_ebysta
    WHEN makenhkh = 'TP' AND thang >= '2025-07-01' AND thang <= '2026-02-28' THEN slkh_ebysta * 1000
    WHEN makenhkh = 'TP' AND thang >= '2025-05-01' AND thang <= '2025-06-30' THEN slkh_ebysta
    WHEN makenhkh = 'TP' AND thang >= '2025-01-01' AND thang < '2025-05-01' THEN slkh_ebysta * 1000
...
LEFT JOIN `staging.d_users` u on
...
END
    ...
    WHEN u.manv = 'QUYNHPTA' then 'OTH_LAB'
    ELSE u.manv
END)
WHERE
date(thang) >= PARSE_DATE('%Y-%m-%d', start_date)
-- AND makenhkh NOT IN ('OTH_LAB')
)

select a.*EXCEPT(trang_thai_giao_hang,pubcustid,pubcustname),
a.makenhkh as makenh_moi,
b.ma_cre,
b.ho_ten_cre,
trang_thai_giao_hang as manvghreal,
---FIELD BỎ
...
a.pubcustname
FROM combined a
LEFT JOIN `spatial-vision-343005.staging.d_calendar_cre` b ON a.manv = b.ma_crs AND date(b.thang) = date(a.thang)
;
-- );
-- Create or replace table `warehouse.f_sales_crs`
-- copy `f_sales_crs`;
END;
