-- ==========================================================================
-- Routine Name : sp_f_data_debt_all_channel
-- Routine Type : PROCEDURE
-- Dataset      : spatial-vision-343005.staging_temp
-- Created      : 2026-06-29 05:41:47.068000+00:00
-- Last Altered : 2026-06-29 05:41:47.068000+00:00
-- Extracted At : 2026-08-06 13:45:04
-- ==========================================================================

CREATE PROCEDURE `spatial-vision-343005`.staging_temp.sp_f_data_debt_all_channel()
BEGIN
CREATE TEMP TABLE `f_data_debt_all_channel_temp` AS
  WITH data_debt AS
     (SELECT CASE
                 WHEN a.channel = 'OTC'
             ...
             CASE
                 WHEN a.so_du_dh > 1000
                      OR a.so_du_dh < -1000 THEN date_diff((SELECT * FROM `staging.d_current_table`), date(a.dateoforder), MONTH)
                 WHEN (a.so_du_dh <= 1000
     ...
     (SELECT channel,
             ...
             sum(CASE
       WHEN thoigian_noqh < 182 THEN no_qh
       ELSE 0
                 END) AS no_phai_thu_pl1_182,
             sum(CASE
       WHEN thoigian_noqh >= 182
                          AND thoigian_noqh < 365 THEN no_qh
       ELSE 0
                 END) AS no_phai_thu_pl2_365,
             sum(CASE
       WHEN thoigian_noqh >= 365
                          AND thoigian_noqh < 730 THEN no_qh
       ELSE 0
                 END) AS no_phai_thu_pl3_730,
             sum(CASE
       WHEN thoigian_noqh >= 730
                          AND thoigian_noqh < 1095 THEN no_qh
       ELSE 0
                 END) AS no_phai_thu_pl4_1095,
             sum(CASE
       WHEN thoigian_noqh >= 1095 THEN no_qh
       ELSE 0
                 END) AS no_phai_thu_pl5_1095
      FROM data_debt
      GROUP BY 1),
        doanhso_hientai AS
 ...
 sum(doanhsochuavat) AS doanhso
      ...
      AND a.thang >='2023-01-01'
      WHERE date(thang) = date_trunc(
                               (SELECT *
                                        FROM `staging.d_current_table`),MONTH)
...
date_trunc(a.orderdate, MONTH) = date_trunc(
                               (SELECT *
                                               FROM `staging.d_current_table`),MONTH)
      GROUP BY 1),
        doanhthu_bq AS
...
CREATE OR REPLACE TABLE `warehouse.f_data_debt_all_channel` COPY `f_data_debt_all_channel_temp`; END;
