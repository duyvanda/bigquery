CREATE VIEW `spatial-vision-343005.warehouse.view_f_mua_hang_sp_benita_xylo_nhom`
AS with 
...
select 
a.*,
round(ds/he_so_nhom,1) as tb_ds_nhom,
round(sl_kh/he_so_nhom,1) as tb_pp_nhom,
Case when round(sl_kh/he_so_nhom,1) >=20 then 'Đạt' else 'Không đạt' end as is_dk_pp,
Case when round(sl_kh/he_so_nhom,1) >=20 then rank() over (partition by round(sl_kh/he_so_nhom,1) >=20,khu_vuc order by round(ds/he_so_nhom,1) desc,ds desc)
...
'' as manv,
'' as tencvbh,
Case when rank_nhom = 1 then 5000000 else 0 end as giai_thuong
 from rank_nhom_t;