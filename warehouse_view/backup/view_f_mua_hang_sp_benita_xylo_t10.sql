CREATE VIEW `spatial-vision-343005.warehouse.view_f_mua_hang_sp_benita_xylo_t10`
AS with 
...
Case 
     ...
     when ten_crm ='Nguyễn Văn Án' then 9
     ...
     when ten_crm ='Lê Duy Chung' then 11.5
...
a.ten_crm,
a.khu_vuc,
count(distinct makhdms_dat) as sl_kh,
sum(soluong) as soluong,
sum(ds) as ds 
 from mapping_phanphoi a 
 group by all
)
,
...
select a.*,
Case 
    when position ='D' and sl_kh < 15 then 'Không đạt'
    when position ='D' and sl_kh >= 15 then 'Đạt'
    when sl_kh >= 30  then 'Đạt' 
else 'Không đạt' end as is_dk_pp,
Case when  sl_kh >= 30  then 'Đạt' else 'Không đạt' end as is_dk_pp_rank_ca_nhan,

 from summary_crs_crm a 
)
,
rank_top250 as 
(
  select *,
  Case when is_dk_pp_rank_ca_nhan ='Đạt'  then rank() over (partition by is_dk_pp_rank_ca_nhan ='Đạt',khu_vuc order by a.ds desc,sl_kh desc )
else null end as rank_online_t9,
   from mapping_dk a 
  
)

-- select * from mapping_phanphoi where manv ='MR3181'
select 
a.*,
Case when is_dk_pp = 'Đạt'  then 500000 else 0 end as giai_thuong,
Case 
  when khu_vuc ='MB' and rank_online_t9 < 9 then 2000000 
  when khu_vuc ='MN' and rank_online_t9 < 8 then 2000000 
else 0 end as giai_thuong_ca_nhan,
b.chucdanhengtitlesum,
b.chucdanhengtitle
 from rank_top250 a
 LEFT JOIN `staging.d_hr_dsns` b on a.manv =b.msnvcsmmoi
 order by a.ds desc,sl_kh desc;